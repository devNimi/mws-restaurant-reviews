(function() {
  ...
  self.addEventListener('sync', function(event) {
    ...
    if (event.tag == 'reviewSync') {
      ...
      event.waitUntil(
        sendMessageToClient({ message: 'post-offline-reviews-to-server' })
      );
    }
    if (event.tag == 'toggleFavouriteSync') {
      console.log(
        'sync event fired for pending favourite prefrence that user added while offline'
      );
      event.waitUntil(
        sendMessageToClient({
          message: 'post-offline-favourites-to-server'
        })
      );
    }
  });
